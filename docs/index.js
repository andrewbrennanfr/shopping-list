...
var fetchData = (fetchArg, storageArg) => {
  ...
  return request(url).then((response) => response.text()).then((body) => {
    const json = JSON.parse(body.slice(47).slice(0, -2));
    const rows = json.table.rows.map(({ c }) => c.map((cc) => cc?.v || "")).filter((row) => row.some(Boolean));
    const amounts = [
    ...
    const units = [...new Set(rows.map(([, , , unit]) => unit))].map((type, i) => ({ id: `u_${i}`, type }));
    const recipes = rows.reduce((recipes2, [recipe, ingredient, amount, unit]) => {
      const amountId = amounts.find(({ value }) => value === parseFloat(amount))?.id;
      const ingredientId = ingredients.find(({ name }) => name === ingredient)?.id;
      const unitId = units.find(({ type }) => type === unit)?.id;
      if (!amountId || !ingredientId || !unitId)
        return recipes2;
      ...
      if (recipe)
        return [
          ...
          {
            contents: [content],
            id: `r_${recipes2.length}`,
            name: recipe
          }
        ];
      const lastRecipe = recipes2.at(-1);
      if (lastRecipe)
        return [
          ...recipes2.slice(0, -1),
          {
    ...
    const categories = rows.map(([, , , , category]) => category).filter((category) => category).map((name, i) => ({ name, recipeId: `r_${i}` })).reduce((categories2, { name, recipeId }) => {
      const contents = [recipeId];
      if (categories2.find((category) => category.name === name)) {
        return categories2.map((category) => category.name === name ? {
          ...category,
          contents: [
            ...category.contents,
            ...contents
      ...
      return [
        ...categories2,
        { contents, id: `c_${categories2.length}`, name }
      ];
    }, []);
